...
class BunnyCachedLogoImage extends StatelessWidget {
  ...
  const BunnyCachedLogoImage(
      {Key? key, required this.logoUrl, required this.title})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return CachedNetworkImage(
        ...
        imageUrl: logoUrl,
        placeholder: (context, url) => Text('${title.substring(0, 1)}',
            style: AppTypography.header.copyWith(fontWeight: FontWeight.bold),
            textAlign: TextAlign.center),
        errorWidget: (context, error, stacktrace) {
          Fimber.w('Failed to load brand logo: $title, st: $stacktrace', ex: error);
          return Text('${title.substring(0, 1)}',
              style: AppTypography.header.copyWith(fontWeight: FontWeight.bold),
              textAlign: TextAlign.center);
        },
